...
# Record errors in array
results = numpy.empty((0,11))

# Take n trials to evaluate error bounds
for n in range(1,100):
    # Gaussian noise array
    # center around 0 and just guess at standard deviation as it was not
    # reported by Eagleworks
    mu, sigma = 0.0, 0.1  # mean and standard deviation for noise
    ...
    perturb = numpy.random.normal(0, 0.1, 1)[0]
    ...
    result = numpy.array([0.004615 - cal1Top.slope, 1249.360 - cal1Top.intercept, 0.005096 - cal1Bot.slope,
              1248.367 - cal1Bot.intercept, -0.07825 - cal2Top.slope, 1263.499 - cal2Top.intercept,
              -0.0827 - cal2Bot.slope, 1263.163 - cal2Bot.intercept,
              0.13826 - f_pulse.slope, 1245.238 - f_pulse.intercept,
              106 - val / dx_df])

    results = numpy.append(results, [result],axis=0)


print('Calc','Cal1T_m','Cal1T_b','Cal1B_m','Cal1B_b','Cal2T_m','Cal2T_b',
                    'Cal2B_m','Cal2B_b','F_m','F_b','Force')

print("Stdev")
print(numpy.std(results,axis=0))
print ("average")
print(numpy.average(results,axis=0))
print("min")
print(numpy.min(results,axis=0))
print("max")
print(numpy.max(results,axis=0))